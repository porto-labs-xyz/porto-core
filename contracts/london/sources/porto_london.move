module porto_london::porto_london {
    ...
    #[test(publisher = @porto_london, administrator = @0xa, coordinator = @0xb)]
    fun initializes_with_the_test_asset_binding(publisher: signer, administrator: signer, coordinator: signer) {
        initialize(&publisher, signer::address_of(&administrator), signer::address_of(&coordinator), @0x1, 100, 4);
        assert!(settlement_asset() == @0x1, 100);
    }

    #[test(publisher = @porto_london, administrator = @0xa, coordinator = @0xb, stranger = @0xc)]
    #[expected_failure(abort_code = E_NOT_ADMINISTRATOR)]
    fun rejects_unapproved_period_administrator(publisher: signer, administrator: signer, coordinator: signer, stranger: signer) {
        initialize(&publisher, signer::address_of(&administrator), signer::address_of(&coordinator), @0x1, 100, 4);
    ...
    fun rejects_unapproved_usage_coordinator(publisher: signer, administrator: signer, coordinator: signer, stranger: signer) {
        initialize(&publisher, signer::address_of(&administrator), signer::address_of(&coordinator), @0x1, 100, 4);
    ...
    #[test(publisher = @porto_london, administrator = @0xa, coordinator = @0xb)]
    fun accepts_an_exact_usage_retry(publisher: signer, administrator: signer, coordinator: signer) {
        initialize(&publisher, signer::address_of(&administrator), signer::address_of(&coordinator), @0x1, 100, 4);
        open_funding_period(&administrator, b"period-a", 10, b"policy");
    ...
    fun rejects_a_conflicting_usage_retry(publisher: signer, administrator: signer, coordinator: signer) {
        initialize(&publisher, signer::address_of(&administrator), signer::address_of(&coordinator), @0x1, 100, 4);
        open_funding_period(&administrator, b"period-a", 10, b"policy");
        submit_usage_batch(&coordinator, b"period-a", b"lane-a", 1, 2, b"bucket-a", b"digest-a", 42);
    ...
    fun rejects_an_overlapping_lane_range(publisher: signer, administrator: signer, coordinator: signer) {
        initialize(&publisher, signer::address_of(&administrator), signer::address_of(&coordinator), @0x1, 100, 4);
        open_funding_period(&administrator, b"period-a", 10, b"policy");
        submit_usage_batch(&coordinator, b"period-a", b"lane-a", 1, 2, b"bucket-a", b"digest-a", 42);
    ...
    fun rejects_usage_for_a_closed_period(publisher: signer, administrator: signer, coordinator: signer) {
        initialize(&publisher, signer::address_of(&administrator), signer::address_of(&coordinator), @0x1, 100, 4);
        open_funding_period(&administrator, b"period-a", 10, b"policy");
